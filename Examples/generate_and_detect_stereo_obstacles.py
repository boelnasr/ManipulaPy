...
class ScenePerception:
    def __init__(self):
        ...
        # Add calibration to improve camera-to-world transformation
        self.calibrate_camera_transformation()
    ...
    def _setup_cameras(self):
        cam_height = 1.0
        cam_distance = 1.5  # Moved closer to objects
        baseline = 0.3     # Increased baseline for better depth resolution
        ...
        self.camera_configs = [left_cam, right_cam]
    ...
    def calibrate_camera_transformation(self):
        """
        Calibrate the camera-to-world transformation using known objects.
        """
        # In a real-world scenario, you would use markers with known positions
        # For simulation, we can use the spawned objects
        calibration_points = []
        
        for obj_id in self.spawned_objects[:min(3, len(self.spawned_objects))]:
            # Get object position in world frame
            world_pos, _ = pb.getBasePositionAndOrientation(obj_id)
            
            # Capture stereo image and get point
            ...
            # Find closest point in cloud to world position
            # This is a simplification - in practice you'd use fiducial markers
            calibration_points.append({
                "camera_point": np.mean(point_cloud, axis=0),  # Simplified
                "world_point": world_pos
            })
        
        # Store calibration for later use
        self.calibration_points = calibration_points
        logger.info(f"✅ Calibration completed with {len(calibration_points)} points")

    def transform_camera_to_world(self, points, camera_translation, camera_rotation):
        """
        Apply a rigid body transformation from camera to world coordinates.
        Args:
            points (Nx3): Points in camera frame
            camera_translation (list): [x, y, z]
            camera_rotation (list): [roll, pitch, yaw] in degrees
        Returns:
            Nx3: Points in world coordinates
        """
        # Convert to radians and get rotation matrix
        rot_matrix = R.from_euler('xyz', camera_rotation, degrees=True).as_matrix()
        transformed = points @ rot_matrix.T 
        return transformed
    ...
    def _process_stereo_vision(self):
        ...
        logger.info("🔄 Transforming point cloud to world coordinates...")
        camera_translation = self.camera_configs[0]["translation"]
        camera_rotation = self.camera_configs[0]["rotation"]
        filtered_cloud = self.transform_camera_to_world(
            filtered_cloud,
            camera_translation,
            camera_rotation
        )
        logger.info("✅ Point cloud transformed to world coordinates")
        ...
        labels, num_clusters, processed_cloud = self._dbscan_clustering(
            filtered_cloud,
            min_samples=10,
            min_cluster_volume=0.001,
            max_cluster_volume=3.0,
        ...
        merged_labels = self._merge_nearby_clusters(processed_cloud, labels, max_merge_distance=0.25)
    ...
    def _dbscan_clustering(
        self, cloud, min_samples=10,
        min_cluster_volume=0.001, max_cluster_volume=3.0,
        min_points_per_cluster=80, max_points_per_cluster=10000
    ):
        """
        ...
        try:
            ...
            eps = avg_distance * 0.6  # Reduced multiplier for tighter clusters
        except Exception as e:
    ...
    def _statistical_outlier_removal(self, cloud, k=50, std_ratio=1.5):
        """
        Remove statistical outliers from point cloud with more stringent parameters.
        """
        if len(cloud) < k + 1:
            return cloud
    ...
    def _make_obstacle_array(self, cloud, labels, num_clusters):
        ...
        for label in valid_labels:
            ...
            mapped_x = self._map_value(centroid[0], detected_x_range, expected_x_range)
            mapped_y = self._map_value(centroid[1], detected_y_range, expected_y_range)
            mapped_z = self._map_value(centroid[2], detected_z_range, expected_z_range)
