...
def main():
    ...
    trajectory = trajectory_planner.joint_trajectory(thetastart, thetaend, Tf, N, method)
    ...
    simulation.run_controller(controller, joint_positions, joint_velocities, joint_accelerations, g, Ftip, Kp, Ki, Kd)

    # Plot the trajectory in the simulation scene
    # Forward kinematics to get end-effector positions
    ee_trajectory = [ur5.forward_kinematics(joint_positions[i])[:3, 3] for i in range(N)]
