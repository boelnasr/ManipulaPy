...
class TestKinematics(unittest.TestCase):
    ...
    def test_end_effector_velocity_space(self):
        ...
        V_ee = self.robot.end_effector_velocity(
            self.test_angles, dthetalist, frame="space"
        )
    ...
    def test_end_effector_velocity_body(self):
        ...
        V_ee = self.robot.end_effector_velocity(
            self.test_angles, dthetalist, frame="body"
        )
    ...
    def test_end_effector_velocity_invalid_frame(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.robot.end_effector_velocity(
                self.test_angles, dthetalist, frame="invalid"
            )
        self.assertIn("Invalid frame specified", str(context.exception))
    ...
    def test_inverse_kinematics_with_plot(
        self,
        mock_use,
        mock_plot,
        mock_xlabel,
    ...
    def test_inverse_kinematics_damping_parameter(self):
        ...
        solution_high, success_high, iter_high = (
    ...
    def test_velocity_consistency(self):
        ...
        max_error = max(
            np.linalg.norm(analytical_velocity), np.linalg.norm(numerical_velocity)
        )
        ...
        try:
            ...
            relative_error = (
                absolute_error / max_error if max_error > 1e-10 else absolute_error
            )
