...
# Module-qualified keys: a bare function name is not unique across the package,
# so a same-named kernel in the later module would overwrite the earlier entry
# and hide a real edit to it. See tests/_kernel_digest.py.
EXPECTED_HASHES = {
...
def test_raw_cuda_kernel_source_hashes_across_modules():
    ...
    assert actual.keys() == EXPECTED_HASHES.keys()
