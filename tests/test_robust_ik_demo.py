...
This is intentionally a demo script, not a pytest test module. Keep the
expensive robot loading and IK loops behind ``main()`` so pytest collection can
import the file without doing demo work.
"""
...
def main():
    ...
    print(
        f"\nSuccess rate: {successes_iterative}/{num_tests} "
        f"({rate_iterative:.1f}%)\n"
    )
    ...
    for i, T_target in enumerate(targets):
        ...
        if success:
            ...
            print(
                f"  Test {i + 1}: SUCCESS "
                f"({total_iters} total iters, strategy={strategy})"
            )
        else:
            print(f"  Test {i + 1}: FAILED")
    ...
    print(
        ...
        f"({successes_robust - successes_iterative} more successes)"
    )
