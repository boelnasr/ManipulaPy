...
class TestTrajectoryPlanning(unittest.TestCase):
    ...
    def create_mock_objects(self):
        ...
        def patched_init(self, serial_manipulator, urdf_path, dynamics, joint_limits, torque_limits=None):
            ...
            self.potential_field = types.SimpleNamespace(compute_gradient=lambda *args: np.zeros(len(args[0])))
    ...
    def test_joint_trajectory(self):
        ...
        try:
            ...
            np.testing.assert_allclose(trajectory["positions"][0], thetastart, rtol=1e-3)
            ...
            np.testing.assert_allclose(trajectory["velocities"][0], np.zeros_like(thetastart), atol=1e-3)
            np.testing.assert_allclose(trajectory["velocities"][-1], np.zeros_like(thetaend), atol=1e-3)
    ...
    def test_joint_trajectory_quintic(self):
        ...
        try:
            ...
            np.testing.assert_allclose(trajectory["accelerations"][0], np.zeros_like(thetastart), atol=1e-3)
    ...
    def test_cartesian_trajectory(self):
        ...
        try:
            ...
            # Check start and end positions
            np.testing.assert_allclose(trajectory["positions"][0], Xstart[0:3, 3], rtol=1e-3)
            np.testing.assert_allclose(trajectory["positions"][-1], Xend[0:3, 3], rtol=1e-3)
    ...
    def test_inverse_dynamics_trajectory(self):
        ...
        try:
            ...
            for i in range(self.num_joints):
                self.assertTrue(np.all(torques[:, i] >= self.trajectory_planner.torque_limits[i, 0]))
                self.assertTrue(np.all(torques[:, i] <= self.trajectory_planner.torque_limits[i, 1]))
    ...
    def test_forward_dynamics_trajectory(self):
        ...
        if self.num_joints > 3:
            self.skipTest("Skipping forward dynamics test for complex robot to avoid numerical issues")
