    ...
    def test_perception_logger_setup(self):
        ...
        try:
            ...
            perception = Perception(
                vision_instance=self.mock_vision, logger_name="TestLogger"
            )
...
class TestPerceptionClustering(unittest.TestCase):
                ...
                for label in unique_labels:
                    ...
                    if len(cluster_points) > 1:
                        distances = np.linalg.norm(
                            cluster_points - cluster_points[0], axis=1
                        )
                        self.assertTrue(
    ...
    def test_clustering_with_mock_sklearn(self):
        ...
        try:
            ...
            labels, num_clusters = perception.cluster_obstacles(
                test_points, eps=0.1, min_samples=3
            )
            ...
            self.assertIsInstance(
                num_clusters, int, "Should return integer cluster count"
            )
    ...
    def test_clustering_empty_points(self):
        ...
        try:
            ...
            labels, num_clusters = perception.cluster_obstacles(
                np.empty((0, 3)), eps=0.1, min_samples=3
            )

            self.assertEqual(
                len(labels), 0, "Should return empty labels for empty input"
            )
            self.assertEqual(
                num_clusters, 0, "Should return zero clusters for empty input"
            )
...
class TestPerceptionStereoMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.stereo_vision.compute_disparity.return_value = np.random.randn(
            480, 640
        ).astype(np.float32)
        self.stereo_vision.get_stereo_point_cloud.return_value = np.random.randn(
            1000, 3
        )
    ...
    def test_stereo_disparity_computation(self):
        ...
        try:
            ...
            self.stereo_vision.rectify_stereo_images.assert_called_once_with(
                left_img, right_img
            )
    ...
    def test_stereo_point_cloud_generation(self):
        ...
        try:
            ...
            self.stereo_vision.get_stereo_point_cloud.assert_called_once_with(
                left_img, right_img
            )
...
class TestPerceptionIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_pb["getCameraImage"].return_value = (
            width,
            height,
            rgba,
            depth,
    ...
    def test_perception_with_real_vision_fixed(self):
        ...
        try:
            ...
            obstacle_points, labels = perception.detect_and_cluster_obstacles(
                camera_index=0, depth_threshold=3.0, step=5, eps=0.1, min_samples=3
            )

            # Verify results
            self.assertIsInstance(
                obstacle_points, np.ndarray, "Obstacles should be numpy array"
            )
            self.assertIsInstance(labels, np.ndarray, "Labels should be numpy array")

            if len(obstacle_points) > 0:
                self.assertEqual(
                    obstacle_points.shape[1], 3, "Obstacles should be 3D points"
                )

            # Verify PyBullet was called
            self.assertTrue(
                self.mock_pb["getCameraImage"].called, "Camera image should be captured"
            )
...
class TestPerceptionErrorHandling(unittest.TestCase):
    ...
    def test_clustering_parameter_validation(self):
        ...
        try:
            ...
            labels, num_clusters = perception.cluster_obstacles(
                test_points, eps=0.5, min_samples=3
            )
            ...
            labels, num_clusters = perception.cluster_obstacles(
                test_points, eps=0.01, min_samples=1
            )
...
if __name__ == "__main__":
    ...
    print(
        f"- Vision module: {'✓' if is_module_available('ManipulaPy.vision') else '✗'}"
    )
