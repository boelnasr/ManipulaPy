...
new_initial_thetalist = np.array(
    [pi, pi / 6, pi / 4, -pi / 3, -pi / 2, (-2 * pi / 3)]
) + np.random.normal(0, 0.2, 6)

# Perform inverse kinematics with the new initial guess
thetalistd = ur5.iterative_inverse_kinematics(
    T_space, new_initial_thetalist, plot_residuals=False
)

print("\nInverse Kinematics with Adjusted Initial Guess (Space Frame):")
print(thetalistd)
T_d = ur5.forward_kinematics(
    np.array(
        [3.14579196, 0.56079453, 0.71427021, -1.00188078, -1.56211583, -2.1069051]
    ),
    frame="space",
)
print("\nForward Kinematics (Space Frame):")
